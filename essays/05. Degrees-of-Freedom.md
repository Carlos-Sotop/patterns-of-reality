...
## When degrees of freedom entered economics (1930s–1970s)

In the 1920s and 1930s, economists were increasingly confident that economies could be steered through centralized policy. National income accounting, introduced in the 1930s, made it possible to measure aggregates like GDP, employment, and inflation. For the first time, the economy appeared *legible*.

This *legibility* created a temptation: if you could measure the system, you could control it, but that's an ilussion, and the first major challenge to this belief came from Friedrich Hayek.

In 1945, Hayek published *The Use of Knowledge in Society*. His argument was not philosophical but structural. Economic knowledge, he argued, is not centralized. It is fragmented, local, time-specific, and often tacit. Prices are not just numbers, but signals that compress vast amounts of dispersed information about preferences, constraints, and alternative uses of resources.

Hayek did not speak in terms of *degrees of freedom*, but the structure of the argument is identical. An economy evolves along an enormous number of independent axes: local constraints, individual preferences, expectations, technologies, and opportunities. Central planners, no matter how sophisticated, operate in a lower-dimensional space because they see aggregates. The economy, however, moves in particulars.

Planning fails not primarily because planners are corrupt or incompetent, but because the system they attempt to regulate has more degrees of freedom than their instruments can represent.

Despite this warning, postwar macroeconomics moved in the opposite direction. Governments adopted Keynesian demand management. Econometric models estimated stable relationships between inflation, unemployment, and output. The economy was treated as a system with a small number of controllable dimensions. Policy appeared to work, so long as behavior remained roughly unchanged.

In the 1970s, advanced economies experienced *stagflation*: high inflation and high unemployment at the same time. The models predicted this combination should not exist. Policymakers did not just get the numbers wrong, the structure of the system had changed.

In 1976, Robert Lucas Jr. formalized why, in what became known as the **Lucas Critique**. His insight was not that models were poorly estimated, but that they were dimensionally incomplete. Macroeconomic models relied on parameters inferred from past behavior, but policy itself changes incentives, and when incentives change, people adapt, and when behavior adapts, the effective degrees of freedom of the system expand.

In other words, the act of control increases the dimensionality of what is being controlled.

A policy that treats human behavior as fixed is operating in a lower-dimensional model than the system it attempts to regulate. Once agents respond strategically, the controller loses coverage. The model no longer spans the state space of the economy.

By the late 1970s, it was no longer controversial to say that macroeconomic systems exhibit too many degrees of freedom to be reliably controlled by fixed rules. What remained controversial was the implication: that adding rules, rather than stabilizing the system, often increases instability by provoking adaptation along dimensions the model cannot see.

This is why, historically, expanding regulatory frameworks so often correlates with volatility rather than control. Each new rule constrains behavior along known dimensions, while opening new, unmodeled ones. The system does not become simpler, it becomes harder to predict.

## When systems fail, the most common response is to add rules.

This pattern is visible across domains and centuries. In Roman administration, early legal codes were short and principle-based. As the empire expanded, exceptions accumulated. By the late empire, legal texts grew massive, enforcement uneven, and discretion shifted from law to officials. This complexity did not stabilize the system, but it accelerated breakdown.

The same pattern appears in modern bureaucracies. As organizations grow, they encounter edge cases, and each edge case produces a new rule, but the rule is written to prevent the last failure, not the next one, and at the same time, rules do not increase degrees of freedom, they constrain them.

When a system stops working, the first instinct is to constrain behavior. This response feels rational because rules are cheap to produce, require no redesign, no redistribution of power, and no change in the system. A rule is a low-cost intervention that can be added on top of an existing system without touching its foundations.

So each new rule attempts the same maneuver: project a high-dimensional system onto a lower-dimensional surface, and is not a perfect compression, therefore, information is lost in the projection, and behavior shifts to unregulated dimensions. 

This is why periods of regulatory expansion often coincide with:

- loss of legitimacy.
- rising informal workarounds.
...
- edge cases / second order effects multiply.
- control appears stronger while governability declines.

This is not because rules are bad, but because rules do not add degrees of freedom, and instead constrain them.
...
*Legibility*: The modern, precise use of legibility comes from James C. Scott, in his 1998 book
*Seeing Like a State*. He was explaining why large-scale state projects keep failing. As defined by Scott, Legibility is the process by which a complex social reality is simplified into standardized, measurable categories so that it can be administered from above.